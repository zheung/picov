module.exports = function() {
	...
	return {
		...
		// 	throw '缺少对应的[模式]';
		// },
		async m(list = [1992904, 2036502, 83271108], conn, query) {
			let illusts = [];
			let files = [];
			...
			if(list.length) {
				...
				illusts = await query('SELECT * FROM pixiv.illust WHERE id IN ($r)', [list]);
				files = await query('SELECT * FROM pixiv.file WHERE illust IN ($r)', [list]);
			}
