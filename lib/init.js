	...
	global.soger = (socket) => {
		if(socket)
			return {
				...
				ll: (...args) => L(...args) || socket.emit('log', args.join(' ')),
				rl: (stamp, ...args) => L(...args) || socket.emit('log', args.join(' '), stamp),
				el: (event, ...args) => L(...args) || socket.emit(event, args.join(' ')),
				rc: (stamp, color, ...args) => socket.emit('log', args.join(' '), stamp, color)
			};
		else
			return {
				l: () => undefined, r: () => undefined, e: () => undefined,
				ll: () => undefined, rl: () => undefined, el: () => undefined, rc: () => undefined
			};
	};
