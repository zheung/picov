module.exports = async() => {
	...
	router.get('/proxy/thumb', async(ctx, next) => {
		...
		if(cached) {
			log('缓存', '小图', iid);
			ctx.type = 'image/png';
			ctx.body = fs.createReadStream(path.join(conf.path.cache, `${iid}.png`));
		}
		else {
			let thumbStream = await work.thumb(ctx.query), cacheStream = fs.createWriteStream(path.join(conf.path.cache, `${iid}.png`));
