let down = async(url, iid, proc, socket, ext, ts) => {
	try {
		...
		await new Promise((resolve, reject) => {
			...
			if(getStream && getStream.pipe)
				...
				.on('data', (chunk) => {
					...
					rog(socket, ts, '下载', iid, proc, Math.round(passed * 100 / total)+'%');
				})
				.on('end', () => {
					writeStream.end();
				});
...
module.exports = async(iid, time, socket) => {
	...
	await Promise.map(urls, async(url, now) => {
		...
		while(!await down(url, iid, proc, socket, ext, ts) && tid++ <= retry)
			sog(socket, '下载', iid, proc, '失败', `第(${tid}/${retry})次重试`);
	});
